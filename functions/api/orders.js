...
export async function onRequestPost(context){
  ...
  try{
    ...
    if(payment){const transaction=payment.transaction||{},pay=payment.payment||{};const qr=pay.qr_string||pay.payment_number||"";const paymentPage=`/admin/payment.html?slug=${encodeURIComponent(slug)}&order=${encodeURIComponent(id)}&qr=${encodeURIComponent(qr)}&amount=${encodeURIComponent(pay.total_payment||transaction.amount||amount)}&expired=${encodeURIComponent(pay.expired_at||"")}`;await context.env.DB.prepare(`UPDATE orders SET payment_provider='louvin',payment_reference=?,payment_url=?,updated_at=CURRENT_TIMESTAMP WHERE id=?`).bind(transaction.id||null,paymentPage,id).run();return json({ok:true,orderId:id,amount,status:"pending",paymentProvider:"louvin",paymentReference:transaction.id||null,paymentUrl:paymentPage,paymentQrString:pay.qr_string||null,paymentNumber:pay.payment_number||null,totalPayment:pay.total_payment||transaction.amount||amount,expiredAt:pay.expired_at||null,message:"Order berhasil dibuat. Silakan bayar menggunakan QRIS."},201)}
...
export async function onRequestGet(context){
  ...
  const rows=await context.env.DB.prepare(`SELECT o.id,o.package_code,o.amount,o.customer_name,o.customer_phone,o.referral_code,o.payment_provider,o.payment_reference,o.payment_url,o.status,o.paid_at,o.created_at FROM orders o JOIN sites s ON s.id=o.site_id WHERE s.slug=? AND s.owner_id=? ORDER BY o.created_at DESC`).bind(slug,user.id).all();return json({orders:rows.results||[]});
}
