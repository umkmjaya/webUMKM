...
export async function onRequestPost(context){
  ...
  const user=await getSessionUser(context.request,context.env);if(!user)return json({message:"Silakan login terlebih dahulu."},401);
  let body;try{body=await context.request.json()}catch{return json({message:"JSON tidak valid."},400)}
  const slug=clean(body.slug,80).toLowerCase().replace(/[^a-z0-9-]/g,"");const packageCode=clean(body.packageCode,20).toLowerCase();
  ...
  try{
    ...
    if(payment){const transaction=payment.transaction||{},pay=payment.payment||{};await context.env.DB.prepare(`UPDATE orders SET payment_provider='louvin',payment_reference=?,payment_url=NULL,payment_qr=?,updated_at=CURRENT_TIMESTAMP WHERE id=?`).bind(transaction.id||null,pay.qr_string||pay.payment_number||null,id).run();return json({ok:true,orderId:id,amount,status:"pending",paymentProvider:"louvin",paymentReference:transaction.id||null,paymentUrl:null,paymentQrString:pay.qr_string||null,paymentNumber:pay.payment_number||null,totalPayment:pay.total_payment||transaction.amount||amount,expiredAt:pay.expired_at||null,message:"Order berhasil dibuat. Silakan bayar menggunakan QRIS."},201)}
