...
export async function onRequestGet(context) {
  if (!context.env?.DB) return json({ sites: [], message: "D1 belum terhubung." });
  const { results } = await context.env.DB.prepare(`
    SELECT id, slug, business_name, tagline, package_code, status, custom_domain, created_at
...
export async function onRequestPost(context) {
  ...
  try {
    await context.env.DB.prepare(`
      INSERT INTO sites (id, slug, business_name, tagline, description, whatsapp, city, address, template_id, package_code, status)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'active')
    `).bind(
      id, slug, businessName, body.tagline || "", body.description || "", body.whatsapp || "", body.city || "", body.address || "", templateId, packageCode
    ).run();
